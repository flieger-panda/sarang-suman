...
I styled the interface entirely in Tailwind utility classes, using Tailwind v4's Vite plugin and `@theme` customization instead of a separate config file, and layered in interaction details like arrow-key portfolio navigation, a wave effect that lingers at the screen edge, and load-in animations built with Motion and anime.js. Content authoring is decoupled from the component code: each portfolio entry is a Markdown file sorted by a `date` field in its frontmatter, so adding or reordering projects never touches TypeScript.
